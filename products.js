...
display(getData, getImageTitle);
function display(arr, arr2) {
    document.querySelector("#products").innerHTML = "";
    document.querySelector("#cat_image>img").src = arr2[0];
    document.querySelector("#cat_title").innerText = arr2[1];
    arr.forEach(function (elem) {
        ...
        p.innerText = `You Save ${elem.discount}%`;
...
function handleFilter() {
    ...
    let filterList = getData.filter(function (elem) {

        if (selected == "1kTo3k") {
            return (elem.price > 1000 && elem.price <= 3000);
        }
        if (selected == "3kTo5k") {
            return (elem.price > 3000 && elem.price <= 5000);
        }
        if (selected == "5kToAbove") {
    ...
    display(filterList, getImageTitle);
